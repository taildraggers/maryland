...
Daily aggregator of Maryland-located taildragger classified listings,
...
repos' approach, with the state filter swapped to "MD".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, MD` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Maryland"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/maryland/`
...
```html
<iframe
  src="https://taildraggers.github.io/maryland/"
  title="Taildragger Ads in Maryland"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_MARYLAND_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
