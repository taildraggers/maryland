"""Aggregates Maryland-located listings from every taildraggers.com
...
ones whose location ends in ", MD".
...
Indiana/Iowa/Kansas/Kentucky/Louisiana/Maine repos' approach, with the state
filter swapped to "MD".
"""
...
PAGE_TITLE = "Taildragger Ads in Maryland"
STATE_ABBR = "MD"
...
_MARYLAND_LOCATION_RE = re.compile(r",\s*MD\s*$", re.IGNORECASE)


def _is_maryland(location: str) -> bool:
    return bool(_MARYLAND_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        maryland_rows = [row for row in rows if _is_maryland(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(maryland_rows)} in Maryland")
        all_listings.extend(maryland_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Maryland listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Maryland listings: {len(listings)}")
